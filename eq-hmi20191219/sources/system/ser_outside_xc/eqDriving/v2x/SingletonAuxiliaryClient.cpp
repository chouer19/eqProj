...
#include "header/system/ser_outside_xc/eqDriving/util/Strlib.hpp"
...
string SingletonAuxiliaryClient::header_token(){
    boost::lock_guard<boost::mutex> lock{mutex_};
    return header_token_;
}
void SingletonAuxiliaryClient::header_token(const string &_value){
    boost::lock_guard<boost::mutex> lock{mutex_};
    header_token_ = _value;
}

string SingletonAuxiliaryClient::clearLoadPosition(const string &_vin, const int &_type, const point_3d_t &_coordinate){
    string body_content = "{\n";
    body_content += "\"vin\":\""+ _vin  + "\",\n";
    body_content += "\"type\":\""+ to_string(_type) + "\",\n";
    string coordinate = "";
    coordinate = boost::lexical_cast<string>(_coordinate.x) + ","+boost::lexical_cast<string>(_coordinate.y)+","+boost::lexical_cast<string>(_coordinate.z);
    body_content += "\"coordinate\":\""+ coordinate + "\"";
    body_content += "\n}";
    string response_body = http_post(this->url_ + "/auxiliary/clearLoadPosition", body_content);
    display("clearLoadPosition",body_content, response_body);
    string code = readValueByKey(response_body, "code"); 
    return code;
}

getVehicleTask_respond_t SingletonAuxiliaryClient::getVehicleTask(const string &_vehicleNo){
    string body_content = "{\n";
    body_content += "\"vehicleNo\":\""+ _vehicleNo + "\"";
    body_content += "\n}";
    string response_body = http_post(this->url_ + "/auxiliary/getVehicleTask", body_content);
    display("getVehicleTask",body_content, response_body);
    getVehicleTask_respond_t tmp_respond;
    tmp_respond.code = readValueByKey(response_body, "code"); 
    tmp_respond.task_type = readReturnDataValueByKey(response_body,"taskType");
    tmp_respond.load_state = readReturnDataValueByKey(response_body,"loadState");
    return tmp_respond;
}

string SingletonAuxiliaryClient::stopAroundVehicle(
          const string &_vehicleNo,
          const int &_vehicleType
          ){
    string body_content = "{\n";
    body_content += "\"vehicleNo\":\""+ _vehicleNo + "\",\n";
    body_content += "\"type\":\""+ boost::lexical_cast<string>(_vehicleType) + "\"";
    body_content += "\n}";
    string response_body = http_post(this->url_ + "/auxiliary/stopAroundVehicle", body_content);
    string code = readValueByKey(response_body, "code"); 
    display("stopAroundVehicle",body_content, response_body);
    return code;
}

string SingletonAuxiliaryClient::taskFinish(
          const string &_vin,
    ...
    body_content += "\"taskNo\":\""+_taskNo+ "\"";
    body_content += "\n}";
    string response_body = http_post(this->url_ + "/auxiliary/taskFinish", body_content);
    string code = readValueByKey(response_body, "code"); 
    display("taskFinish",body_content, response_body);
    return code;
}
...
string SingletonAuxiliaryClient::loadoffDispatch(     
    ...
    for(auto vertex : _coordinates){
        coordinates += "" + boost::lexical_cast<string>(vertex.x) + ",";
        coordinates += "" + boost::lexical_cast<string>(vertex.y) + "#";
    }
    body_content += "\"coordinates\":\""+coordinates+ "\",\n";
    body_content += "\"itemType\":\""+boost::lexical_cast<string>(_itemType)+ "\",\n";
    body_content += "\"courseAngle\":\""+boost::lexical_cast<string>(_azimuth)+ "\"";
    body_content += "\n}";
...
string SingletonAuxiliaryClient::loadoffDispatch(     
    ...
    body_content += "\"coordinates\":\""+coordinates+ "\",\n";
    body_content += "\"itemType\":\""+boost::lexical_cast<string>(_itemType)+ "\",\n";
    body_content += "\"latitude\":\""+boost::lexical_cast<string>(_latitude)+ "\",\n";
    body_content += "\"longitude\":\""+boost::lexical_cast<string>(_longitude)+ "\",\n";
    body_content += "\"courseAngle\":\""+boost::lexical_cast<string>(_azimuth)+ "\"";
    body_content += "\n}";
    cout << body_content << endl;
    string response_body = http_post(this->url_ + "/auxiliary/loadoffDispatch", body_content);
...
confirmLoadPlace_respond_t SingletonAuxiliaryClient::confirmLoadPlace(
    ...
    body_content += "\"latitude\":\""+boost::lexical_cast<string>(_latitude) + "\",\n";
    body_content += "\"longitude\":\""+boost::lexical_cast<string>(_longitude) + "\",\n";
    body_content += "\"azimuth\":\""+boost::lexical_cast<string>(_azimuth) + "\",\n";
    string coordinates = "";
    for(auto vertex : _coordinates){
        coordinates += "" + boost::lexical_cast<string>(vertex.x) + ",";
        coordinates += "" + boost::lexical_cast<string>(vertex.y) + "#";
...
confirmLoadPlace_respond_t SingletonAuxiliaryClient::confirmLoadPlace(
    ...
    string isNew= _is_new ? "1":"0";
    body_content += "\"isNew\":\""+isNew+ "\",\n";
    body_content += "\"autoConfirm\":\""+ boost::lexical_cast<string>(_autoConfirm)+ "\",\n";
    body_content += "\"loadType\":\""+ boost::lexical_cast<string>(_loadMode)+ "\",\n";
    string coordinate = "";
    coordinate = boost::lexical_cast<string>(_coordinate.x) + ","+boost::lexical_cast<string>(_coordinate.y)+","+boost::lexical_cast<string>(_coordinate.z);
    body_content += "\"coordinate\":\""+ coordinate + "\"";
    body_content += "\n}";
    ...
    tmp_respond.code = readValueByKey(response_body, "code"); 
    tmp_respond.load_space_id = readReturnDataValueByKey(response_body,"loadPositionId");
    SplitString vertexsString(readReturnDataValueByKey(response_body, "coordinate"));
    vector<string> vertexStrings = vertexsString.split('#');
    for(auto vetexString : vertexStrings){
        SplitString tmpString(vetexString);
        vector<string> contents = tmpString.split(',');
        if(contents.size() != 2){continue;}
        point_2d_t vertex;
        vertex.x =  atof(contents[0].c_str());
        vertex.y =  atof(contents[1].c_str());
        tmp_respond.vertexs.push_back(vertex);
    }
    tmp_respond.latitude = atof(readReturnDataValueByKey(response_body,"latitude").c_str());
    tmp_respond.longitude = atof(readReturnDataValueByKey(response_body,"longitude").c_str());
    tmp_respond.azimuth = atof(readReturnDataValueByKey(response_body,"azimuth").c_str());
...
string utcTimeToString(const utc_time &tm){
    return boost::lexical_cast<string>(tm.year) + "-" + boost::lexical_cast<string>(tm.month) + "-" + boost::lexical_cast<string>(tm.day) + " " +
           boost::lexical_cast<string>(tm.hour) + ":" + boost::lexical_cast<string>(tm.minute) + ":" + 
           boost::lexical_cast<string>(tm.second) + "." + boost::lexical_cast<string>(tm.millisecond);
}
...
//}
loadFinish_respond_t SingletonAuxiliaryClient::loadFinish(const string &_vin, const string &_driver_id, const bool &_is_hard){
    this->vehicleVin(_vin);
    this->driverId(_driver_id);
    return this->loadFinish();
}
loadFinish_respond_t SingletonAuxiliaryClient::loadFinish(const string &_vin, const string &_driver_id, const bool &_is_hard , const point_3d_t &_coordinate){
    this->vehicleVin(_vin);
    this->driverId(_driver_id);
    string code = "";
    string body_content = "{\n";
    ...
    loadFinish_respond_t tmp_respond;
    tmp_respond.code = readValueByKey(response_body, "code"); 
    SplitString vertexsString(readReturnDataValueByKey(response_body, "coordinate"));
    vector<string> vertexStrings = vertexsString.split('#');
    for(auto vetexString : vertexStrings){
...
loadFinish_respond_t SingletonAuxiliaryClient::loadFinish(){
    string code = "";
    string body_content = "{\n";
    body_content += "\"vin\":\""+this->vehicleVin() + ",\n";
    body_content += "\"driver\":\""+this->driverId() +",\n";
...
string SingletonAuxiliaryClient::terminalSystemSet(){
    ...
    body_content += ",\"terminalType\":\""+boost::lexical_cast<string>(this->vehicleType())+"\"";
...
getRequestStatus_respond_t SingletonAuxiliaryClient::getRequestStatus(){
    ...
    string body_content = "{";
    body_content += "\"vin\":\""+this->vehicleVin() +"\"";
    body_content += ",\"faultType\":\""+boost::lexical_cast<string>(this->requestType())+"\"";
...
string SingletonAuxiliaryClient::delayedRequest(){
    ...
    string body_content = "{";
    body_content += "\"vin\":\""+this->vehicleVin() +"\"";
    body_content += ",\"vehicleType\":\""+boost::lexical_cast<string>(this->vehicleType())+"\"";
    body_content += ",\"faultType\":\""+boost::lexical_cast<string>(this->delayType())+"\"";
    body_content += ",\"times\":\""+boost::lexical_cast<string>(this->duration())+"\"";
    body_content += ",\"proposer\":\""+this->driverId()+"\"";
    body_content += "}";
...
string SingletonAuxiliaryClient::faultRequest(){
    ...
    body_content += "\"vin\":\""+this->vehicleVin()+"\"";
    body_content += ",\"vehicleType\":\""+boost::lexical_cast<string>(this->vehicleType())+"\"";
    body_content += ",\"faultType\":\""+boost::lexical_cast<string>(this->faultType())+"\"";
...
string SingletonAuxiliaryClient::updateVehicleStatus(const int &_status,const string &_id){
...
string SingletonAuxiliaryClient::updateVehicleStatus(){
    ...
    string body_content = "{";
    body_content += "\"vin\":\""+this->vehicleVin()+"\"";
    body_content += ",\"driverId\":\""+this->driverId()+"\"";
    body_content += ",\"status\":\""+boost::lexical_cast<string>(this->vehicleStatus())+"\"";
    body_content += "}";
    cout << "request is : \n";
    cout << body_content << endl;
...
login_respond_t SingletonAuxiliaryClient::login(){
    ...
    body_content += "\"vin\":\""+ this->vehicleVin() +"\"";
    body_content += ",\"type\":\""+ boost::lexical_cast<string>(this->vehicleType()) +"\"";
    ...
    point_3d_t gps_xyz;
    point_3d_t spoon_xyz;
    point_3d_t origin_llh;
    double deviationAngle = 0;
gps_xyz.x = atof(readReturnDataValueByKeys(response_body,vector<string>{"vehicleMode","carrierReferenceX"}).c_str());
    gps_xyz.y = atof(readReturnDataValueByKeys(response_body,vector<string>{"vehicleMode","carrierReferenceY"}).c_str());
    gps_xyz.z = atof(readReturnDataValueByKeys(response_body,vector<string>{"vehicleMode","carrierReferenceZ"}).c_str());
    spoon_xyz.x = atof(readReturnDataValueByKeys(response_body,vector<string>{"vehicleMode","targetReferenceX"}).c_str());
    spoon_xyz.y = atof(readReturnDataValueByKeys(response_body,vector<string>{"vehicleMode","targetReferenceY"}).c_str());
    spoon_xyz.z = atof(readReturnDataValueByKeys(response_body,vector<string>{"vehicleMode","targetReferenceZ"}).c_str());
    origin_llh.y = atof(readReturnDataValueByKeys(response_body,vector<string>{"vehicleMode","originLat"}).c_str());
    origin_llh.x = atof(readReturnDataValueByKeys(response_body,vector<string>{"vehicleMode","originLon"}).c_str());
    origin_llh.z = atof(readReturnDataValueByKeys(response_body,vector<string>{"vehicleMode","originHeight"}).c_str());
    deviationAngle = atof(readReturnDataValueByKeys(response_body,vector<string>{"vehicleMode","driftAngle"}).c_str());

    login_respond_t tmp_respond = generate_login_respond(code);
    tmp_respond.gps_xyz = gps_xyz;
    tmp_respond.spoon_xyz = spoon_xyz;
    tmp_respond.origin_llh = origin_llh;
    tmp_respond.deviationAngle = deviationAngle;
    return tmp_respond;
}
...
string SingletonAuxiliaryClient::http_post(const string &url, const string &request_body){
    ...
    try {
        ...
        request << boost::network::header( "Content-Length", boost::lexical_cast<string>(request_body.size()));
        request << boost::network::header("token","totototokenkenkenkenken")
        request << boost::network::header("Connection","close");
        string token = header_token();
        if(token.size() >0){
            request << boost::network::header( "token", token);
        }
        response = client.post(request);
        response_body = body(response);
    
        auto it = response.headers().find("token");
        if(it != response.headers().end()){
            header_token(it->second);
            cout << "header token: " << it->second << endl;
        }
...
string SingletonAuxiliaryClient::readReturnDataValueByKeys(const string &_body, const vector<string> &_keys){
    string value = "";
    try{
        simdjson::ParsedJson pj = simdjson::build_parsed_json(_body);
        simdjson::ParsedJson::Iterator pjh(pj);
        ...
        int index = 0;
        while(index >=0 && index < _keys.size() && pjh.move_to_key(_keys[index++].c_str())){
            pjh.print(cout);cout<<endl;
            if(pjh.is_string()){
...
void SingletonAuxiliaryClient::updateParams(){
    ...
    string SQL = "UPDATE HTTP_PARAMS SET VIN = '" +this->vehicleVin() +"' ,TOKEN = '"+ this->vehicleVin() + string(this->vehicleVin(),this->vehicleVin().size() > 3 ? this->vehicleVin().size()-3:0,3)  +"' ,TYPE="+boost::lexical_cast<string>(this->vehicleType())+" ,TERMINAL_NUMBER = '"+this->terminalNumber()+"',VEHICLE_NUMBER ='" +this->vehicleNumber()+"',SIM = '"+this->simId()+"',PASSWD = '"+this->passwd()+"' ,AuxDomain = '"+this->domain()+"' ,AuxPort = " +boost::lexical_cast<string>(this->port())+"  WHERE ID = 1;"; cout << SQL << endl;
    //sql = SQL.c_str();
