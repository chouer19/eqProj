...
#include "ser_outside_xc/thirdParty/gzip/compress.hpp"
#include "ser_outside_xc/thirdParty/gzip/config.hpp"
#include "ser_outside_xc/thirdParty/gzip/decompress.hpp"
#include "ser_outside_xc/thirdParty/gzip/utils.hpp"
#include "ser_outside_xc/thirdParty/gzip/version.hpp"
...
void SingletonClient::update_gis(GIS_UPDATE_PACKET lidar_data){
    //vector<uint8_t> time = current_time();
    uint8_t *uncompressed_bytes = new uint8_t[8 + 1 + 32 + 1 + 1 + 1 + lidar_data.lidar_data.size()];
    size_t iterator = 0;
...
bool SingletonClient::estop(){
    boost::lock_guard<boost::mutex> lock{mutex};
    this->estop_packet.is_new = false;
    return (this->estop_packet.estop == 0x00);
}
